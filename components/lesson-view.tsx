...
export default function LessonView({ content, author }: { content: GeneratedContent; author?: string | null }) {
  ...
  return <article className="rounded-2xl border border-border bg-white p-6 md:p-8">
    <div className="flex flex-wrap items-center gap-3"><span className="rounded-full bg-secondary px-3 py-1 text-sm font-bold text-study-link">{content.level}</span>{author && <span className="text-sm text-muted-foreground">Do {author} chia sẻ</span>}</div>
    ...
    <div className="mt-8 border-t border-border pt-7">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">{item.options.map((option, index) => <Button key={index} type="button" variant="outline" disabled={checked} aria-pressed={selected === index} onClick={() => setSelected(index)}
        className={`h-auto min-h-14 justify-start whitespace-normal rounded-xl p-4 text-left ${checked && index === item.answerIndex ? "border-[#3a8f69] bg-[#eaf7ef]" : selected === index ? "border-study-link bg-study-tint" : ""}`}>
        <span className="mr-2 font-bold">{String.fromCharCode(65 + index)}.</span><span lang="ja">{option}</span>
      </Button>)}</div>
      {checked && <div role="status" className="mt-5 rounded-xl bg-study-tint p-4 leading-7"><strong>{selected === item.answerIndex ? "Đúng rồi." : "Chưa đúng."}</strong><p className="mt-1">{item.explanation}</p><p className="mt-2 text-sm text-muted-foreground">Nguồn trong tài liệu: {item.sourceHint}</p></div>}
