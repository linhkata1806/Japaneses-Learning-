...
import { documentUploadsEnabled, documentUploadUnavailableMessage } from "@/lib/document-storage";
...
function DocumentsWorkspace() {
  ...
  async function upload(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!documentUploadsEnabled) { setMessage(documentUploadUnavailableMessage); return; }
    if (!file) return;
  ...
  return <div className="min-h-screen bg-background">
    <header className="border-b border-border bg-white"><div className="mx-auto flex max-w-[1200px] items-center justify-between gap-3 px-5 py-4 md:px-8">
      <ManabiBrand compact />
      <AccountControl />
    </div></header>
    <main className="mx-auto max-w-[1200px] px-5 py-8 md:px-8">
      ...
      {!documentUploadsEnabled && <p role="status" className="mt-4 rounded-xl bg-study-tint p-4 text-sm leading-6">{documentUploadUnavailableMessage}</p>}
      {loading ? <p className="mt-8">Đang tải…</p> : !learner ? <div className="mt-8 rounded-2xl border border-border bg-white p-6"><p>Cần đăng nhập để lưu và xử lý tài liệu.</p><a href="/login?returnTo=%2Fdocuments" className="mt-3 inline-block font-semibold text-study-link">Đăng nhập</a></div>
        : <div className="mt-8 grid gap-7 lg:grid-cols-[330px_minmax(0,1fr)]">
          <aside className="space-y-6">
            <form onSubmit={upload} className="rounded-2xl border border-border bg-white p-5">
              ...
              <Input type="file" accept=".pdf,.docx,.txt" onChange={event => setFile(event.target.files?.[0] || null)} disabled={!documentUploadsEnabled} className="mt-4 h-auto py-2" aria-label="Chọn tài liệu" />
              <Button type="submit" disabled={!documentUploadsEnabled || !file || Boolean(busy)} className="mt-4 w-full">Lưu tài liệu</Button>
            </form>
            ...
            {!selectedDoc ? <div className="py-14 text-center text-muted-foreground">Chọn một tài liệu để tạo hoặc chỉnh sửa bài học.</div>
              : <>
                ...
                {!draft && <div className="mt-6">
                  ...
                  <Button type="button" onClick={generate} disabled={!documentUploadsEnabled || !geminiAvailable || !consent || remainingAi === 0 || Boolean(busy)} className="mt-5">{busy === "generate" ? "AI đang xử lý…" : "Tạo bài học bằng AI"}</Button>
...
export default function DocumentsPage() {
  return <RouteAccessGate><DocumentsWorkspace /></RouteAccessGate>;
}
