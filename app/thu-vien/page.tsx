...
export default function LibraryPage() {
  ...
  return <div className="min-h-screen bg-background">
    <header className="border-b border-border bg-white"><div className="mx-auto flex max-w-[1100px] justify-between px-5 py-4"><a href="/" className="font-bold text-[#1c456b]">Manabi<span className="text-[#e5593f]">.</span></a><a href="/" className="text-sm font-semibold text-[#315b85]">← Về luyện tập</a></div></header>
    <main className="mx-auto max-w-[1100px] px-5 py-10">
      ...
      {loading ? <p className="mt-8">Đang tải…</p> : error ? <p role="alert" className="mt-8">{error}</p> : items.length === 0
        ...
        : <div className="mt-8 grid gap-4 md:grid-cols-2">{items.map(item => <a key={item.id} href={`/bai/${item.id}`} className="rounded-2xl border border-border bg-white p-6 transition hover:border-[#789cba] hover:shadow-lg">
          <span className="text-sm font-bold text-[#315b85]">{item.content.level}</span>
