...
export default async function PublicLessonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!row) notFound();
  return <main className="mx-auto max-w-3xl px-5 py-8"><a href="/thu-vien" className="text-sm font-semibold text-[#315b85]">← Thư viện</a><div className="mt-6"><LessonView content={JSON.parse(row.payloadJson) as GeneratedContent} author={row.author} /></div></main>;
}
