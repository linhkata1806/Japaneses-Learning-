...
export default function SharedLesson({ token }: { token: string }) {
  ...
  return <main className="mx-auto max-w-3xl px-5 py-8"><Link href="/" className="text-sm font-semibold text-study-link">← Trang luyện tập</Link><div className="mt-6">{item ? <LessonView content={item.content} author={item.author} /> : <div className="rounded-xl border border-border bg-white p-6"><p>{status}</p><Link href="/auth" className="mt-3 inline-block text-sm font-semibold text-study-link">Đăng nhập nếu bạn chưa có tài khoản</Link></div>}</div></main>;
}
